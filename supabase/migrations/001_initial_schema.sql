-- ============================================================================
-- ByteBuildIT — Initial Database Schema Migration
-- ============================================================================
...
CREATE UNIQUE INDEX idx_departments_org_slug ON public.departments (org_id, slug);
...
CREATE UNIQUE INDEX idx_team_members_org_slug ON public.team_members (org_id, slug);
...
CREATE UNIQUE INDEX idx_pages_org_slug ON public.pages (org_id, slug);
...
CREATE INDEX idx_page_sections_ordering ON public.page_sections (page_id, ordering);
...
CREATE UNIQUE INDEX idx_projects_org_slug ON public.projects (org_id, slug);
...
CREATE INDEX idx_projects_ordering ON public.projects (ordering);
...
CREATE UNIQUE INDEX idx_solutions_org_slug ON public.solutions (org_id, slug);
CREATE INDEX idx_solutions_org_id ON public.solutions (org_id);
...
CREATE UNIQUE INDEX idx_industries_org_slug ON public.industries (org_id, slug);
CREATE INDEX idx_industries_org_id ON public.industries (org_id);
...
CREATE UNIQUE INDEX idx_services_org_slug ON public.services (org_id, slug);
...
CREATE INDEX idx_media_org_id ON public.media (org_id);
CREATE INDEX idx_media_folder_path ON public.media (org_id, folder_path);
...
CREATE INDEX idx_navigation_items_org_id ON public.navigation_items (org_id);
CREATE INDEX idx_navigation_items_parent_id ON public.navigation_items (parent_id);
CREATE INDEX idx_navigation_items_location ON public.navigation_items (org_id, nav_location, ordering);
...
CREATE UNIQUE INDEX idx_settings_org_key ON public.settings (org_id, key);
CREATE INDEX idx_settings_category ON public.settings (org_id, category);
...
CREATE UNIQUE INDEX idx_redirects_org_source ON public.redirects (org_id, source_path);
CREATE INDEX idx_redirects_org_id ON public.redirects (org_id);
...
CREATE INDEX idx_lead_status_history_lead_id ON public.lead_status_history (lead_id, created_at);
...
CREATE INDEX idx_project_media_media_id ON public.project_media (media_id);
...
CREATE INDEX idx_project_industries_industry_id ON public.project_industries (industry_id);
...
CREATE INDEX idx_project_solutions_solution_id ON public.project_solutions (solution_id);
...
CREATE TABLE public.solution_industries (
  solution_id uuid NOT NULL REFERENCES public.solutions(id) ON DELETE CASCADE,
  industry_id uuid NOT NULL REFERENCES public.industries(id) ON DELETE CASCADE,
  PRIMARY KEY (solution_id, industry_id)
);

CREATE INDEX idx_solution_industries_industry_id ON public.solution_industries (industry_id);

-- ============================================================================
-- ROW-LEVEL SECURITY  (RLS)
-- ============================================================================
-- Supabase enables RLS by default on public tables. Uncomment and adjust
-- the policies below to match your auth strategy.
--
-- GENERAL PATTERN:
--   - Service-role / server functions bypass RLS (default Supabase behaviour).
--   - Public (anon) role: read-only on published content.
--   - Authenticated CMS users: full CRUD scoped to their org_id.
--
-- Example skeleton (uncomment and adapt as needed):
--
...
--
-- CREATE POLICY "CMS users can manage their org's pages"
--   ON public.pages FOR ALL
--   USING (
--     auth.uid() IN (
--       SELECT id FROM public.users
--       WHERE org_id = pages.org_id AND is_active = true
--     )
--   );
--
-- Repeat for each table. For organisations the first org created via a
-- seed script should be tied to the deploying user's auth.uid().
--
-- TIP: Enable RLS with  ALTER TABLE <table> ENABLE ROW LEVEL SECURITY;
-- then add policies for SELECT (anon), and ALL (authenticated org member).
--
-- Tables that are 100 % server-side only (audit_logs, lead_status_history,
-- role_permissions) can restrict to service_role only:
--
...
-- ============================================================================
-- SEED DATA (optional — uncomment to bootstrap first org + admin user)
-- ============================================================================
-- INSERT INTO public.organizations (id, name, slug)
--   VALUES ('00000000-0000-0000-0000-000000000001', 'ByteBuildIT', 'bytebuildit');
--
-- INSERT INTO public.roles (id, name, slug, is_system)
--   VALUES
--     ('00000000-0000-0000-0000-000000000010', 'Owner', 'owner', true),
--     ('00000000-0000-0000-0000-000000000011', 'Admin', 'admin', true),
--     ('00000000-0000-0000-0000-000000000012', 'Editor', 'editor', true),
--     ('00000000-0000-0000-0000-000000000013', 'Author', 'author', true);
-- ============================================================================

-- ============================================================================
-- Done. Total: 28 tables, triggers, indexes, and RLS scaffolding.
-- ============================================================================
